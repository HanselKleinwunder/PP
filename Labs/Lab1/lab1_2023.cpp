...
int main()
{
	...
	char cByte = 'A'; //65  'A'  ,		cByte	0x41 'A'	char символьынй тип данных, который представлен 
	//с помощью одного байта (8 бит)

	cByte = 0x42; //0x42 'B'  		cByte	0x42 'B'	char

	cByte = 66;  //0x42 'B'  		cByte	0x42 'B'	char	

	cByte = -1;    //0xff  		    cByte	0xff 'ÿ'	char

	unsigned char ucByte = 0x41;  //	ucByte	0x41 'A'	unsigned char беззнаковый символьный тип данных 
	ucByte = 'B';//		            ucByte	0x42 'B'	unsigned char
	ucByte = -1; //		            ucByte	0xff 'ÿ'	unsigned char


	int iInt = 0xffffffff; //		    iInt	0xffffffff	int целочисленный тип данных, размер которого зависит 
	//от архитектуры процессора. Занимает либо 2, либо 4 байта


	unsigned int uiInt = 0xffffffff; //4294967295	uiInt	0xffffffff	unsigned int


	float fFloat = 1.f;  //0xffffffff 		fFloat	1.00000000	float
	//тип данных с плавающей запятой. В памяти занимает 4 байта (32 бита)


	double dDouble = 1.; //		dDouble	1.0000000000000000	double
	//Так же тип с плавающей запятой. Занимает 8 байт (64 бита)
	...
	stop
		...
		wchar_t cw = L'Ф';	   //cw	0x0424 'Ф'	wchar_t 		cw	1060 'Ф'	wchar_t
	size_t n = sizeof(cw); //0x00000002 		n	0x00000002	unsigned int
	...
	stop
		...
		iInt = 1;
	double dDouble1 = static_cast<double>(iInt / 3);		//(1)
	//dDouble1	0.0000000000000000	double

	double dDouble2 = static_cast<double>(iInt) / 3.;	// (2)
	//dDouble2	0.33333333333333331	double
	//1. Сначала мы делаем целочисленное деление, а полученный ноль преобразуем в дабл.
	//2. Сначала явно приводим тип инт к дабл, затем делим на 3.0
	//3. В первом случае мы получим целочисленный результат, а во втором дробный
	...
	//1. Оператор правоассоциативен, то приоритет начинается справа.
	//2. Первое действие: 3.3 является типом данных дабл.
	//	 При делении на целочисленный тип, мы отбрасываем дробную часть. 
	//	 Результат выражения 3.3/3 присваивается в символьный тип данных char.
	//	 Итог: результатом является единица, мы потеряли дробную часть 0.1
	//3. Четвёртое действие: мы опять же результат одного типа данных присваиваем в переменную другого типа данных
	//	 float - тип данных с плавающей запятой, а int - целочисленный тип данных.
	//   Итог: При присваивании 1 в тип float, мы не получаем дробную часть 1.0
	//4. Результатом будет 1, т.к. типу double было присвоено значение 1.
	//	 Были потеряны данные, а именно дробная часть числа.

	cByte = (sShort = (iInt = (fFloat = (dDouble = 3.3 / 3))));			// (2)
	//1. При попытке преобразовать double во float мы рискуем потерять данные,
	//   т.к. второй тип занимает 4 байта, когда первый - 8
	...
	cByte = iInt;							//Hexadecimal Display:  0x01 '\x1'

	unsigned char cN1 = 255, cN2 = 2, cSum;
	cSum = static_cast<unsigned char>(static_cast<int>(cN1) + static_cast<int>(cN2));
	//1. Выход за разрядную сетку. Чар занимает в памяти 1 байт, а диапазон значений, который он может хранить = от 0 до 255.
	//   В данном случае, мы к 255, т.е. к максимально-возможному значению, прибавляем ещё 2,
	//   в следствии чего получаем выход за разрядную сетку
	...
	// Напишите явно преобразования, которые неявно выполняет компилятор
	int iSum = static_cast<int>(cN1) + static_cast<int>(cN2);
	//1. В данном случае не происходит выход за разрядную сетку, потому что символьные типы мы преобразуем в интовые.
	//   Инт, в зависимости от разрядности процессора, занимает, либо 2, либо 4 байта.
	//   В сумме мы имеем число намного меньшее, чем необходимо, чтобы выйти за пределы интовой разрядной сетки
	...
	int iSum1 = static_cast<int>(c1) + static_cast<int>(c2);   //?(1)
	int iSum2 = uc1 + uc2; //?(2)
	//1. 0xff в знаковом представлении = -1. Естественно, -1 + 2 = 1.
	//2. 0xff в беззнаковом представлении = 255. 255 + 2 = 257.
	...
	//1. Ответ: 36.3. Сначала компилятор выполнит деление. Делим на целочисленное, поэтому дробную часть потеряем.
	//   Затем, посредством плюса получаем 36.3


	//Получите результат без потери точности с помощью оператора явного
	//приведения типа
	double dDouble3 = 3.3 + nTmp / static_cast<double>(nn); //dDouble3 = 36.633333333333333
...
// *******************************************************
	...
	TwinSpace::nTwin = 300; //Определена в своём пространстве имён TwinSpace
	nTwin += 1;	//Обращаемся к глобальной переменной. К текущему значению этйо переменной прибавляем единицу 101
	...
	int nTwin;		//Локальная переменная внутри функции main
	nTwin = 200;	//Присваиваем локальной переменной значение 200
	::nTwin = ::nTwin + 1;	//Глобальной переменной добавляем единицу  102
	{
		int nTwin;	//Объявляем новую локальную переменную внутри текущего блока
		nTwin = -1;	//Минусуем единицу у локальной переменной внутри текущего блока
		::nTwin += 1;	//Добавляем единицу к глобальной переменной   
		TwinSpace::nTwin += 1;	//Добавляем единицу переменной из пространства имён TwinSpace. Получаем 301.
	}

	nTwin -= 1;	//Отнимаем единицу у локальной переменной текущего блока. Получаем 199.

	// Задание 3.2

	//В этом фрагменте фигурируют  три переменных с одним и тем же именем
	// nTest - глобальная (определена в другом модуле). 
	// Вторая - локальная внутри main,  третья - локальная внутри блока

	nTest = 1000; //Глобальная переменная
	int nTest = -2000; //Локальная переменная внутри main. Инициализируем и присваеваем ей значение -2000
	stop
	{
		int nTest = 200; //Инициализируем переменную, и присваеваем ей значение 200
		nTest += 2;	     //К текущей переменной со значение 200 прибавляем 2. Получаем 202
		stop
	}
	nTest += 2;			//Локальной переменной внутри main прибавляем 2. Получаем -1998
...
// *******************************************************
	...
	for (int i = 0; i < 3; i++)
	{
		static int nStatic1 = 100; //Статические переменные с каждой итерацией не инициализируются заново
		int nLocal1 = 100;
		int nLocal2 = 200;
	...
	//А. Выведите на каждой итерации значения nLocal1, nLocal2, nStatic1, nStatic2  на консоль  и заполните табличку
	//	 nLocal1 =  1: 101, 2: 101, 3: 101
	//	 nLocal2 =  1: 201, 2: 200, 3: 200
	//	 nStatic1 = 1: 101, 2: 102, 3: 103
	//	 nStatic2 = 1: 403, 2: 404, 3: 405
	//Б. Инициализация переменных выполняется, когда мы заходим в сам цикл.
	//В. Во время инициализации nStatic1, программист сам присваивает значение
	//	 Во время инициализации nStatic2, присваивание происходит в момент вычисления выражения
	...
	for (int i = 0; i < 3; i++)
	{
		static int nStatic1;  //В момент инициализации по дефолту было присовено значение 0, т.к. программист явно не задал
		nStatic1 = 100; //Тут происходит операция присваивания
		int nLocal1 = 100;
		nStatic1 += 1;
		nLocal1 += 1;
		stop

			//А. Когда переменной nStatic1 значение было указано не в момент инициализации,
			//   она с каждой итерацией цикла присваивала значение 100.
			//Б. Переменная nStatic1 инициализирована без значения.
			//Выведите на каждой итерации значения nLocal1, nStatic1 на консоль  и заполните табличку:
			//nLocal1 = 1: 101, 2: 101, 3: 101
			//nStatic1 = 1: 101, 2: 101, 3: 101
	}
	...
	myColors = static_cast<eColors>(123); //static_cast всегда справа от знака равенства
	stop
...
// *******************************************************
...

	{
		...
		const int nN = 1; //При инициализации константной переменной, мы должны сразу же присвоить ей значение,
		//т.к. в последствии компилятор не позволит нам изменить его
		//nN = 1; //Мы не можем присвоить значение константной переменной после того, как она была инициализирована
